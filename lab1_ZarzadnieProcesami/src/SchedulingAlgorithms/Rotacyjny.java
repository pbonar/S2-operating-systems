...
import Processor.MemoryDone;
import Processor.MemoryToAppear;
import Processor.MemoryToDo;
import Processor.SingleProcess;
...
public class Rotacyjny {
    ...
    public float algorithmRotacyjny(ArrayList<SingleProcess> toAppear, int oneTime){
        ...
        while (memoryToDo.getLength() != 0 || memoryToAppear.getLength() != 0){
            ...
            if (i != -1){
                int oneTimee;
                if (memoryToDo.getLista().get(i).getTimeLeft() < oneTime && memoryToDo.getLista().get(i).getTimeLeft() > 0) {
                    oneTimee = memoryToDo.getLista().get(i).getTimeLeft();
                } else {
                    oneTimee = oneTime;
                }
